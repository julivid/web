                                ...
                                if (empty($ticketList)) {
                                    ...
                                    foreach ($ticketList as $ticket) {
                                        echo '<tr class="">
                                            ...
                                            <td>'.($ticket['ticket_status']==1?'<a href="javascript:;" onclick="getDetailInfo('.$ticket['uid'].')" class="btn btn-small">详细</a>':'-').'</td>
                                        </tr>';
                                    }
                                }
                                ?>
...
function getDetailInfo(uid) {
    if (!uid) {
        Materialize.toast('用户ID错误', 2000, 'danger');
        return;
    }
    infoClear();
    $.ajax({
        url: '<?php echo base_url("Account/getTicketDetailInfoAJAX");?>',
        data:{ id:uid, a:Math.random()},
        type: "POST",
        dataType: "json",
