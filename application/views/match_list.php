                    ...
                    if (empty($matchList)) {
                        ...
                        foreach ($matchList as $k=>$match) {
                            ...
                            echo '<tr class="'.($match['is_del']==0?'':'match-detele').'">
                                ...
                                <td><a class="btn btn-small" href="'.base_url('Match/downloads?id='.$match['mid']).'" target="_blank">下载名单</a></td>
                                ...
                                <td>';
                            if ($match['status']==0) {
                                echo '<a class="btn btn-small green" href="javascript:;" onclick="updateMatch('.$match['mid'].', 1, this)">发布</a> ';
                            }else{
                                echo '<a class="btn btn-small orange" href="javascript:;" onclick="updateMatch('.$match['mid'].', 0, this)">取消</a> ';
                            }
                            echo ' <a class="btn btn-small" href="'.base_url('Match/edit?id='.$match['mid']).'">修改</a> ';
                            if ($match['status'] < 2) {
                                echo '<a class="btn btn-small red" href="javascript:;" onclick="updateMatch('.$match['mid'].', 9, this)">删除</a>';
                            }
                            echo '</td>
                            </tr>';
                        }
...
function formatMatchData(data, obj) {
    ...
    var htmlStr = '';
    var _MatchStatusStyle = [['disabled', '未发布'], ['default', '报名中'], ['info', '进行中'], ['warning', '已结束'], ['warning', '截止报名']];
    var editUrl = '<?php echo base_url("Match/edit?id=");?>';
    $.each(data, function(i,o){
        htmlStr += '<tr '+(o.is_del==0?'':'class="match-detele"')+'><th>'+o.mid+'</th><td>'+o.type_name+'</td><td>'+o.name+'</td><td>'+o.date+'</td><td><label class="MS_'+_MatchStatusStyle[o.status][0]+'">'+_MatchStatusStyle[o.status][1]+'</label></td><td><a class="btn btn-small" href="javascript:;">下载名单</a></td><td id="MatchInfo_'+o.mid+'" data-gt="'+o.game_type+'" data-ec="'+o.entry_ticket+'" data-sc="'+o.start_condition+'" data-prize="'+o.prize+'" data-de="'+o.delayed_enroll+'" data-logo="'+o.logo+'" data-dt="'+o.date+' '+o.time+'"><a class="btn btn-small " href="javascript:;" onclick="showDetail('+o.mid+')">配置</a> <a class="btn btn-small green" href="javascript:;">牌桌</a></td><td><a class="btn btn-small " href="'+editUrl+o.mid+'">修改</a> <a class="btn btn-small red" href="javascript:;">删除</a></td></tr>';
    });
